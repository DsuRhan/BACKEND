...
import { validate, getStoreByIdValidation, createStoreValidation } from "../middlewares/store.validation.js";
const router = Router();
const repo = new StoreRepository();
const controller = new StoreController(new GetAllStoresService(repo), new GetStoreById(repo), new CreateStore(repo), new UpdateStore(repo), new DeleteStore(repo));
/**
 * @swagger
 * tags:
 *   name: Stores
 ...
 */
router.put("/stores/:id", validate(getStoreByIdValidation.concat(createStoreValidation)), controller.update);
/**
 * @swagger
 * /stores/{id}:
 *   delete:
